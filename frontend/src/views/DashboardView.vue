...
const statCards = computed(() => [
  { icon: Document, color: '#409eff', label: '报表总数', value: reports.value.length, suffix: '份' },
  { icon: TrendCharts, color: '#67c23a', label: '本月新建', value: 12, suffix: '份' },
  { icon: Files, color: '#e6a23c', label: '模板数量', value: 3, suffix: '个' },
  { icon: Upload, color: '#909399', label: '本月导出', value: 8, suffix: '次' },
]);

const quickActions = [
  { icon: '📝', title: '创建日报', desc: '快速生成本日工作报表', path: '/reports/create', query: { type: 'daily' } },
  { icon: '📊', title: '创建周报', desc: '汇总本周工作内容', path: '/reports/create', query: { type: 'weekly' } },
...
<template>
  <section>
    <div class="page-header">
      <div><h2>工作台</h2><p class="muted">欢迎使用 ReportFlow AI 智能报表平台</p></div>
      <el-button type="primary" :icon="Plus" @click="router.push('/reports/create')">创建报表</el-button>
    </div>

    <el-row :gutter="16" style="margin-bottom:20px">
      <el-col v-for="a in quickActions" :key="a.title" :span="6">
        <el-card shadow="hover" class="quick-card" @click="router.push({ path: a.path, query: a.query })">
          <span class="qc-icon">{{ a.icon }}</span>
          <div><strong>{{ a.title }}</strong><p class="muted" style="margin:2px 0 0;font-size:12px">{{ a.desc }}</p></div>
        </el-card>
      </el-col>
    </el-row>
      ...
      <el-col v-for="card in statCards" :key="card.label" :span="6">
        <el-card shadow="never" class="stat-card">
          <div class="stat-row">
            ...
            <div class="stat-info">
              <span class="stat-num">{{ card.value }}<small>{{ card.suffix }}</small></span>
    ...
    <el-card shadow="never" v-loading="reportsLoading">
      <template #header>
        <div class="card-hd"><strong>最近报表</strong><el-button size="small" text type="primary" @click="router.push('/reports')">查看全部</el-button></div>
      </template>
      ...
      <div v-else class="report-list">
        <div v-for="r in reports" :key="r.id || r.title" class="report-row" @click="router.push('/reports/1/edit')">
          <div><span class="r-title">{{ r.title }}</span><span class="muted r-date">{{ r.report_date }}</span></div>
          <div class="r-meta">
...
.quick-card :deep(.el-card__body) { display:flex; align-items:center; gap:14px; padding:16px; }
...
.stat-num small { font-size:13px; font-weight:400; margin-left:2px; }
...
.report-row { display:flex; justify-content:space-between; align-items:center; padding:10px 12px; border-radius:6px; cursor:pointer; transition:background .15s; }
.report-row:hover { background:#f5f7fb; }
